...
const store = createStore({
    state: {
        user: {
            data: {},
            token: sessionStorage.getItem("TOKEN"), // Test Data
        },
        reviews: [...tmpReviews],
    ...
    actions: {
        saveReview({ commit }, review) {
            ...
            if (review.id) {
                ...
                response = axiosClient.post("/review", review).then((res) => {
                    commit("saveReview", res.data);
                    return res;
                });
            }
    ...
    mutations: {
        saveReview: (state, review) => {
            state.reviews = [...state.reviews, review.data];
        },
        updateReview: (state, review) => {
            state.reviews = state.reviews.map((s) => {
                if (s.id === review.data.id) {
                    return review.data;
                }
                return s;
            });
